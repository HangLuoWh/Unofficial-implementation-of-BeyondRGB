...
class Trainer():
    def __init__(self, hyper):
        ...
        self.total_patches = self.dataset_len*hyper['TRAIN']['NUMBER'] # 进入训练的图像块数量
        self.val_ld = self.define_validation_loader()
        self.loss = MyLoss(hyper) # 损失函数
        ...
        for epoch in range(self.start_epoch, self.hyper['TRAIN']['EPOCH']):
            ...
            with tqdm(total=batch_num, leave=False, desc=f'Epoch {epoch + 1}/{self.hyper["TRAIN"]["EPOCH"]}') as pbar:
                for _, batch in enumerate(self.train_ld):
                    ...
                    batch_loss = 0 # 记录一个批次的损失之和
                    batch_image_num = gt_spd.size(0) # 获取一个批次内图像的数量
                    ...
                    for i in range(self.hyper['TRAIN']['NUMBER']):
                        ...
                        epoch_total_loss += loss_sum # 将本批样本的损失和叠加到轮总体损失中，便于计算一轮的平均损失
                        batch_loss += loss_sum # 将本批样本的损失和叠加到批总体损失中，便于计算一轮的平均损失
                    ...
                    pbar.update(1)
            # 记录epoch级日志
            avg_total = epoch_total_loss / self.total_patches # 整个轮次上的平均误差
        ...
        with torch.no_grad():
            with tqdm(total=n_val, desc='validation') as pbar:
                for batch in self.val_ld:
                    ...
                    total_loss_sum += loss_sum # 将本批样本的损失和叠加到总体损失中，便于计算一轮的平均损失
                    pbar.update(1)
    ...
    def define_train_loader(self):
        ...
        self.dataset_len = len(dataset) # 增加属性记录训练样本的数量
    ...
    def define_validation_loader(self):
        ...
        self.vali_len = len(dataset) # 增加属性记录验证集内的样本数量
